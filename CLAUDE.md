...
- **MSVC 链接面 strncasecmp（5fee8cf，run 35937029875 Windows job LNK2019）**：e5734cd 的 compat 桥接头 + `tzset/tzname` 宏注入全部生效（编译段干净过），链接期暴露下一个 POSIX 假设——`strptime.c:90` 全 vendored 源码唯一 `strncasecmp` 使用点。修复：`strncasecmp=_strnicmp` 对象宏映射（沿既有 CMake 注入机制），三重核对后才落笔：声明经 strptime.c include 的 `<string.h>` 可达、确为唯一使用点、同签名 `(const char*, const char*, size_t)→int` 同语义
- **macOS /dev/full 门控（fd25e7e）**：`/dev/full` 是 **Linux 特有**字符设备（写入恒返 ENOSPC）——macOS/BSD 没有，ofstream 打开即 ENOENT → 失败落「文件打开失败」分支（错误消息 `Failed to open output file`）而非「预分配失败」分支，断言钉死 `pre-allocation` 错误消息的 `PreallocDiskFullFailsCleanly`/`FallocDiskFullFailsCleanly`/`TruncDiskFullFailsCleanly` 三用例红。原门控 `#ifndef _WIN32`（注释自述 "POSIX only"）把 macOS 误当有 /dev/full 的平台。修复：门控收窄 `#if defined(__linux__)` + 顺带删除被证伪的注释（「macOS 走 fcntl F_PREALLOCATE 失败」——前提 /dev/full 存在不成立）；同文件 `DiskWriteFailureOn*` 三用例断言宽松（FAILED + 错误非空）两种失败路径下都过，照常真跑零改动。失败语义本身在 macOS 成立（组 FAILED 无假绿），物理不可测沿「测试设施平台依赖」skip 先例（Upnp 真实 IGD / V1 mTLS Schannel 同款）
- **macOS SIGPIPE 进程级投递（1f44064，run 35940031281 macOS 首次真跑曝光）**：`TlsRequestWriteFailFailsCleanly (SIGPIPE)` 0.1s 死——服务器进程整体被信号杀死。定性：Linux 遵循 POSIX「同步信号投递给生成线程」，accept 线程 `pthread_sigmask(SIG_BLOCK, SIGPIPE)` 足够；**macOS XNU 的 socket 层以 psignal 做进程级投递——写线程的线程级屏蔽不隔离，任一未屏蔽线程（gtest 主线程）照样被杀**。修复：TlsTestServer::start() 开头进程级 `signal(SIGPIPE, SIG_IGN)`（POSIX 分支）+ 补 `<csignal>` include——写失败以 EPIPE 返回值出现（OpenSSL 按写失败正常处理），与 edges 测试文件服务器 start() 既有先例同法；accept 线程的 pthread_sigmask 保留（纵深无害）。该共享头 5 个消费 TU（redirect/handler_edges/ipv6/injection/tls）一次修复全覆盖
- **Windows qsort 顺序断言（1b19dd5，同 run Windows job）**：`MetalinkParseTest.EqualRankAllNoPriority` L139 失败——断言与用例注释自相矛盾（注释自述「组内顺序不钉(库 qsort 不稳定)，只断言集合与 rank」，断言却钉 `first`/`second` 确切顺序）。库 `metalink_pctrl.c:238` 按标准 qsort 排序，C 标准不保证同 rank（两 URL 均无 priority）组内顺序；**glibc qsort 对 2 元素数组走插入排序恰好保序 → Linux/macOS 恒绿系统性掩盖，MSVC CRT qsort 不保序 → Windows 暴露**。修复：断言改集合式（沿同文件垃圾 priority 用例既有正确形态）；全文件顺序断言核查（urls[] 9 处 + files[] 2 处逐点 rank 唯一性核对）确认唯一违规即本处——files 列表库不排序（vendored 源码仅 2 处 qsort 均作用 resources/metaurls），文档序断言合法
- **测量级教训**：① 「三平台 CI 全绿」的测试面真相——历史批次的 macOS/ubuntu build job 只验证了**编译**，绿得异常快（40s 级）+ 步骤明细 success 不是测试证据，必须拉日志核对测试数量或 `--no-tests=error` 结构性防呆；② MSVC 红面按层暴露：编译面（include 行/标识符，e5734cd）→ 链接面（符号，5fee8cf）→ **测试面（断言语义，1b19dd5）**，每个修复 commit 只挡住当前层，下一层红面必然再来——vendored 库的 POSIX 假设清单要按层预扫（unistd.h/tzname/alloca → strncasecmp/strtok_r 等链接符号）；③ 平台门控粒度：`#ifndef _WIN32` ≈ "非 Windows" ≠ "POSIX 全集有此设施"——写门控前核实设施的准确覆盖面（/dev/full 是 Linux 特有、非 POSIX 通用）；④ **信号投递语义平台分叉**：「线程屏蔽足够」是 Linux 语义，macOS XNU 对 SIGPIPE 进程级投递——依赖线程级 pthread_sigmask 隔离写信号的多线程测试，macOS 上必须进程级 `signal(SIGPIPE, SIG_IGN)` 兜底（每个做 OpenSSL/裸 socket 写的服务器线程都要有）；⑤ **「注释自述集合断言、代码钉顺序」的自相矛盾用例是平台暴露面**——顺序敏感断言只对唯一 rank（唯一 priority/排序键）合法，同 rank 多元素必须集合断言；C 标准 qsort 不稳定，glibc 恰好保序的绿是运气不是契约
- **验证**：终验 run 35943273520（1b19dd5，七修复合集）**8/8 job 全绿**——strncasecmp 链接修复与 /dev/full 门控经 run 35940031281 确认生效（Windows 编译/链接段干净过、macOS 无 DiskFull 三红），SIGPIPE 免疫与 qsort 集合断言经本 run 确认生效（macOS build `TlsRequestWriteFailFailsCleanly` 不再进程级死亡、Windows build `EqualRankAllNoPriority` 转绿），gtest 假绿收口后 ubuntu gcc/clang 双编译器测试首次真跑全绿；本机 SIGPIPE 修复后 30 轮压测 + TLS/Proxy/Redirect/IPv6 60 用例 + falcon_http_tests 全量绿，qsort 修复后 MetalinkParseTest 55/55 绿
...
- **Metalink3 格式考古三条（红面逐层闭环，手写解析器宽松遍历从不暴露的库状态机门禁，已全部写进测试注释固化）**：① v3 的 `<file>` 必须包在 `<files>` 容器内（v4 直挂根下）；② v3 的 `<url>` 必须包在 `<resources>` 容器内；③ **v3 url 元素 type 属性必需**——缺 type 被库静默跳过（resources 为空非报错）。v3 用例三条不齐即落「不含 file 元素」/「无可用 HTTP/FTP 镜像」两类错误
- **库行为差异面（测试按实际语义钉死）**：库 skip 语义——缺 name 或 check_safe_path 失败（`../` 前缀/裸 `..`/反斜杠/DOS 盘符）的 file 静默跳过非报错，唯一 file 被跳过时落「不含 file 元素」；**check_safe_path 放行嵌入 '/' 的 "sub/dir"**（只查最后一段）→ 由我方 is_safe_filename 二次检查拒绝（「路径穿越」消息，两类拒绝可区分）；**库 qsort 不稳定**——同 priority 组内顺序未定义，测试对同 rank 组集合断言；**strtol 垃圾前导容忍**——"12abc"→priority 12（与手写解析器全串拒绝语义不同）；**v4 的 type 属性库不解析**（mu.type 恒空）——type 过滤靠 scheme 兜底，v4 type="bittorrent" 的 http url 照常采用（与 v3 的 type 过滤形成双版本对照用例）；size 经库 strtoll 垃圾/溢出/负数→0
- **错误码映射（throw_metalink_error）**：METALINK_ERR_BAD_ALLOC(901)→「metalink 解析内存分配失败」；MISSING_REQUIRED_ATTR(101)→「file 元素缺少 name 属性」；PARSER_ERROR(201)→XmlParseError（理论不可达——expat 预检先拦）；default→runtime_error + metalink_strerror
- **测试矩阵（净增 2：90 > 88，铁律「测试只增不减」字面执行）**：mini_xml_parser_test 35 用例随对象整删；metalink_parse_test 18 → **55**（新增 16 个补用例钉回格式层能力：v3 容器层级三条 + v4 type 双形态对照 + 多 file 部分 skip + size 垃圾文本 + 空白 hash 跳过 + 同算法双 hash 全收 + hash 大小写保留 + 空 url 跳过 + url 实体解码 + v3/v4 扩展属性忽略 + 注释/PI 忽略 + CDATA in url）；metalink_handler_test 35 不变（5 处 meta4 文档补 xmlns 修复——手写解析器时代 namespace 缺省宽容掩盖）；**教训：删被测对象必然删其用例，「测试只增不减」必须按净用例数核对而非「新文件用例更多」的错觉——39 用例初稿比 18 翻倍仍净减 14**
- **验证**：build-ci parse 55/55 + e2e 35/35；ASan 三套件 90 用例零告警；protocols 全量绿（3 skip 设计内）+ http_tests 全量绿；build-cov 全量 ctest **2452 清单 2451 过**（1 失败为 BlackHoleServerTimesOutViaEngineFallback 负载抖动惯犯，串行复跑 2.67s 即过 + protocols 全量 1116 重跑恢复 gcda；15 skip 设计内）
- **铁账（build-cov 单树新鲜数据）**：分母 17988 → **17724**（−264 = mini parser .cpp/.hpp/.gcno 出分母 −282 + metalink_handler 重写 +18），行 miss 478 → 491（+13 = metalink_handler 新代码防御面 29 + 时序窗口自然抖动），行 **97.34% → 97.23%**（−0.11 点 = 新代码增量稀释，沿 NAT 批次 −0.33 点/A3「新代码增量稀释」先例）；**分支 miss 15185 → 15069（净收 116 改善）**，分母 35316 → 34826，分支率 57.00% → 56.70%（比率下降纯因删除的 mini parser 集合自身分支覆盖率高拉低总体，算术效应非覆盖回退）；函数 98.8%。metalink_handler 29 miss 逐行定性：throw_metalink_error 错误码簇 12 行（PARSER_ERROR 结构不可达——expat 预检先拦；MISSING_REQUIRED_ATTR 库 skip 语义下不可达——缺 name 被静默跳过不产生该错误码；BAD_ALLOC 不可注入；default 不可达）+ 桥接竞速窗口 ~10 行（resume 失败/pause 转发/publish 25ms 轮询/parent 状态翻转）+ 桥接异常 catch 与产物缺失防御 ~7 行——与 e2e 35 用例全绿自洽。**测量级教训**：删源文件不清 build 树的旧 gcno，gcov 对无 gcda 对象静默输出全零报告（全 miss 冒充真实缺口，分母虚胖 282）——删被测对象必须同步清理其构建产物再跑铁账
- 下一个替换候选：ICE/STUN → libjuice（调研已完成，阶段二才实现）
